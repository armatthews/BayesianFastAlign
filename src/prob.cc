...
int main(int argc, char** argv) {
  ...
  ("reverse,r", "Reverse alignments. Use this iff you used -r with bayesianfastalign.")
  ...
  for (unsigned n = 0; n < sentence_count; ++n) {
    ...
    for (unsigned j = 0; j < link_counts[n].size(); ++j) {
      ...
      unsigned sum = 0;
      for (unsigned i = 0; i < link_counts[n][j].size(); ++i) {
      ...
      unsigned null_count = iteration + 1 - burn_in - sum;
      if (null_count > 0) {
        if (!reverse) {
          cout << (first ? "" : " ") << "NULL" << "-" << j << "/" << null_count;
        }
        else {
          cout << (first ? "" : " ") << j << "-" << "NULL" << "/" << null_count;
        }
      }
    }
    cout << endl;
